...
<body class="no-skin">
		...
		<div class="main-content">
			<div class="main-content-inner">
				<div class="page-content">
					<div class="row">
						<div class="col-xs-12">
							<form action="unitInfo/excel.do" method="post" id="frm2">
								<input type="hidden" name="unitName" value="${h.unitName }"/>
								<input type="hidden" name="unitMemCode" value="${h.unitMemCode }"/>
								<input type="hidden" name="unitLinkMan1" value="${h.unitLinkMan1 }"/>
								<input type="hidden" name="size" value="${ p.total}"/>
							</form>
							<!-- 检索  -->
							<form action="unitInfo/goto.do" method="post" name="Form"
								id="Form">
								...
								<table style="margin-top: 5px;">
									<tr>
										...
										<td>
											<div class="nav-search">
												<span class="input-icon"> <input type="text"
													placeholder="请输入单位名称" class="nav-search-input"
													id="nav-search-input" autocomplete="off" name="unitName"
													value="${h.unitName }" /> <i
										...
										<td>
											<div class="nav-search">
												<span class="input-icon"> <input type="text"
													placeholder="请输入助记码" class="nav-search-input"
													id="nav-search-input" autocomplete="off" name="unitMemCode"
													value="${h.unitMemCode }" />
												</span>
											</div>
										</td>
										<td>
											<div class="nav-search">
												<span class="input-icon"> <input type="text"
													placeholder="请输入联系人" class="nav-search-input"
													id="nav-search-input" autocomplete="off"
													name="unitLinkMan1" value="${h.unitLinkMan1 }" />
												</span>
											</div>
										</td>
								...
								<table id="simple-table"
									...
									<tbody>
										...
										<c:forEach items="${requestScope.list }" var="k" varStatus="i">
											<c:if test="${k.delstatus == 'Y' }">
												<tr>
													...
													<td class='center'>${k.unitname}</td>
													<td class='center'>${k.unitmemcode}</td>
													<td class='center'>${k.unitlinkman1}</td>
													<td class="center">
														<div class="hidden-sm hidden-xs btn-group">
															<a class="btn btn-xs btn-success" title="查看"
																href="unitInfo/look.do?id=${k.id }"> <i
																class="ace-icon fa fa-eye bigger-120" title="查看"></i>
															</a> <a class="btn btn-xs btn-success" title="编辑"
																href="unitInfo/update.do?id=${k.id }"> <i
																class="ace-icon fa fa-pencil-square-o bigger-120"
																title="编辑"></i>
															</a> <a class="btn btn-xs btn-danger"
																href="unitInfo/delete.do?id=${k.id }&delstatus=N"
																onclick="return confirm('请确认是否删除?')"> <i
	...
	<script type="text/javascript">
		window.jQuery
				|| document
						.write("_$tag______________________________________________________________"
								+ "_$tag_______");
	</script>
	<!-- <![endif]-->
	<!--[if IE]>
	...
	<script type="text/javascript">
		...
		//新增
		function add() {
			window.location.href = "unitInfo/add.do";
		}
			...
			bootbox
					.confirm(
							...
							function(result) {
								if (result) {
									top.jzts();
									var url = "http://localhost:9999/FHSHGL/customer/delete.do?CUSTOMER_ID="
											+ Id
											+ "&tm="
											+ new Date().getTime();
		...
		//修改
		function edit(Id) {
			//window.location.href = "unitInfo/update.do";
		}

		//查看
		function view(Id) {
			//window.location.href = "unitInfo/look.do?id="+Id;
		}
