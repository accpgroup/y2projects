...
<body class="no-skin">
		...
		<div class="main-content">
			<div class="main-content-inner">
				<div class="page-content">
					<div class="row">
						<div class="col-xs-12">
							<form action="goodInfo/excel.do" method="post" id="frm2">
								<input type="hidden" name="gdname" value="${h.gdname }"/>
								<input type="hidden" name="gdmemcode" value="${h.gdmemcode }"/>
								<input type="hidden" name="gdsort" value="${h.gdsort }"/>
								<input type="hidden" name="gdnorms" value="${h.gdnorms }"/>
								<input type="hidden" name="size" value="${ p.total}"/>
							</form>
							
							<!-- 检索  -->
							<form action="goodInfo/goto.do" method="post" name="Form"
								id="Form">
								<input type="hidden" name="page" id="page"/>
								<input type="hidden" name="size" id="size" value="${p.pageSize }"/>
								<table style="margin-top: 5px;">
									<tr>
										<td>
											<div class="nav-search">
												<span class="input-icon"> <input type="text"
													placeholder="这里输入商品名" class="nav-search-input"
													id="nav-search-input" autocomplete="off" name="gdname"
													value="${h.gdname }" /> <i
										...
										<td>
											<div class="nav-search">
												<span class="input-icon"> <input type="text"
													placeholder="这里输入助记码" class="nav-search-input"
													id="nav-search-input" autocomplete="off" name="gdmemcode"
													value="${h.gdmemcode }" />
												</span>
											</div>
										</td>
										<td>
											<div class="nav-search">
												<span class="input-icon"> <input type="text"
													placeholder="这里输入商品类别" class="nav-search-input"
													id="nav-search-input" autocomplete="off"
													name="gdsort" value="${h.gdsort }" />
												</span>
											</div>
										</td>
										<td>
											<div class="nav-search">
												<span class="input-icon"> <input type="text"
													placeholder="这里输入商品型号" class="nav-search-input"
													id="nav-search-input" autocomplete="off"
													name="gdnorms" value="${h.gdnorms }" />
											</span>
											</div>
										</td>
									...
									<tbody>
										...
										<c:forEach items="${list }" var="k"  varStatus="i">
											<tr>
												...
												<td class='center'>${k.gdname }</td>
												<td class='center'>${k.gdmemcode }</td>
												<td class='center'>${k.gdsort }</td>
												<td class='center'>${k.gdnorms }</td>
												<td class="center">

													<div class="hidden-sm hidden-xs btn-group">
														<a class="btn btn-xs btn-success" title="查看"
															href="goodInfo/look.do?id=${k.gdid }"
														...
														<div class="inline pos-rel">
															...
															<ul
																...
																<li><a style="cursor: pointer;"
																	href="goodInfo/update.do?id=${k.gdid }
																	class="
																	tooltip-success" data-rel="tooltip" title="修改"> <span
																		class="green"> <i
	...
	<script type="text/javascript">
		window.jQuery
				|| document
						.write("<script src='http://localhost:9999/psi/static/ace/js/jquery.js'>"
								+ "<"+"/script>");
	</script>
	<!-- <![endif]-->
	<!--[if IE]>
	...
	<script type="text/javascript">
		if ('ontouchstart' in document.documentElement)
			document
					.write("<script src='http://localhost:9999/psi/static/ace/js/jquery.mobile.custom.js'>"
							+ "<"+"/script>");
	</script>
	...
	<script type="text/javascript">
		...
		//新增
		function add() {
			window.location.href = "goodInfo/add.do";
		}
		...
		function chaImg(CUSTOMER_ID) {
			...
			diag.Drag = true;
			diag.Title = "跟踪记录";
			diag.URL = 'http://localhost:9999/psi/customerimg/list.do?CUSTOMER_ID='
					+ CUSTOMER_ID;
			diag.Width = 800;
			diag.Height = 600;
		...
		function del(id) {
			bootbox.confirm("确定要删除吗?",
				function(result) {
				if(result){
					window.location.href="goodInfo/delete.do?gdid="+id+"&delstatus=N";		
				}
				});
		}
		...
		//修改
		function edit(Id) {
			window.location.href = "goodInfo/update.do";
		}
